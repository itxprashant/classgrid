...
export default function MyCalendar() {
    ...
    const gridDates = useMemo(() => weeks.flat().map((c) => c.date), [weeks]);
    ...
    }, [showClasses, viewYear, viewMonth]);
    ...
    const openCreatePicker = (dateKey, date) => {
        setModal({ type: 'pick', dateKey, label: fmtFull(date) });
    };

    const chooseCreateMode = (mode) => {
        setModal((prev) => {
            if (!prev || prev.type !== 'pick') return prev;
            return {
                type: 'form',
                        ...
                        {week.map(({ date, inMonth }) => {
                            ...
                            return (
                                <button
                                    ...
                                    type="button"
                                    className={cellClass}
                                    onClick={() => openCreatePicker(key, date)}
                                    aria-label={`Add event on ${fmtFull(date)}`}
                                >
                                            ...
                                            {dayEvents.slice(0, MAX_CELL_EVENTS).map((e) => (
                                                ...
                                                >
                                                    {e.isPersonal ? (
                                                        ...
                                                        <span className="mycal__chip-code mono">
                                                            {e.courseCode}
                                                        </span>
                                                    ) : null}
                                                    <span className="mycal__chip-title">{e.title}</span>
                                                    {formatEventSchedule(e) && (
                                                        <span className="mycal__chip-time mono">
                                                            {formatEventSchedule(e)}
                                                        </span>
                                                    )}
                                                </span>
                    ...
                    >
                        {modal.type === 'pick' ? (
                            <>
                                ...
                                <p className="mycal__modal-date mono">{modal.label}</p>
                                <div className="mycal__picker-options">
                                    <button
                                        type="button"
                                        className="mycal__picker-option"
                                        onClick={() => chooseCreateMode('shared')}
                                    >
                                        <span className="mycal__picker-option-title">Course event</span>
                                        <span className="mycal__picker-option-desc">
                                            Shared with everyone in that course — quiz, deadline, exam, …
                                        </span>
                                    </button>
                                    <button
                                        type="button"
                                    ...
                                    >
                                        <span className="mycal__picker-option-title">Personal event</span>
                                        <span className="mycal__picker-option-desc">
                                            Private to you — not linked to a course
                                        </span>
                                    </button>
                                </div>
                            </>
