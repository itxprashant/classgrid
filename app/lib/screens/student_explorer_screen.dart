...
class StudentExplorerScreen extends StatefulWidget {
  const StudentExplorerScreen({super.key});

  @override
  State<StudentExplorerScreen> createState() => _StudentExplorerScreenState();
}

class _StudentExplorerScreenState extends State<StudentExplorerScreen> {
  ...
  Future<void> _search(String raw) async {
    ...
    try {
      final results = await context.read<HistoryApi>().searchStudents(q);
  ...
  void _openStudent(StudentSummary student) {
    pushAppRoute<void>(
      context,
      StudentDetailScreen(
        ...
        children: [
          TextField(
            ...
            decoration: InputDecoration(
              hintText: 'e.g. sharma or mt6240685',
          ...
          if (_controller.text.trim().length < 2 && !_loading) ...[
            ...
            EmptyState(
              message: 'Type at least 2 characters to search enrollment records.',
              icon: Icons.person_search_outlined,
            ),
          ] else if (!_loading && _results.isEmpty && _controller.text.trim().length >= 2) ...[
            const SizedBox(height: T.space24),
            EmptyState(message: 'No students matched “${_controller.text.trim()}”.', icon: Icons.search_off),
          ] else ...[
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(row.kerberos, style: AppText.mono(size: T.fs12, color: T.ink3)),
                              ],
                            ),
                          ),
                          Pill('${row.enrollmentCount}', tint: T.accentTint, edge: T.accentEdge, ink: T.accentInk),
                        ],
                      ),
                    ),
                  ),
